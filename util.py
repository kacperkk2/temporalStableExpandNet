...
def random_tone_map():
    ...
    return TRAIN_TMO_DICT[tmos[choice]](randomize=True)


def execute_tone_map(tmo, x):
...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, index):
        dpoint_hdr = cv2.imread(
            self.file_list[index], flags=cv2.IMREAD_ANYDEPTH + cv2.IMREAD_COLOR
        )
        dpoint_hdr = cv2.resize(dpoint_hdr, (512, 512))
        dpoint_hdr = map_range(dpoint_hdr)
        ldr_name = self.file_list[index][:-4] + ".png"
        dpoint_ldr = cv2.imread(
            ldr_name, flags=cv2.IMREAD_ANYDEPTH + cv2.IMREAD_COLOR
        ).astype(np.float32)
        dpoint_ldr = cv2.resize(dpoint_ldr, (512, 512))
        dpoint_ldr = map_range(dpoint_ldr)

        return cv2torch(dpoint_ldr), cv2torch(dpoint_hdr)
